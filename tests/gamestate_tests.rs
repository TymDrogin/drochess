...
mod tests {
    ...
    fn test_square_new_from_file_rank() {
        let square = Square::new_from_file_rank(3, 4);

        assert_eq!(square.get_index(), 35); // 4 << 3 + 3 = 32 + 3 = 35

        let square = Square::new_from_file_rank(7, 7);
    ...
    fn test_set_for_side() {
        ...
        assert_eq!(rights.get(), (CastlingSide::Kingside as u8) | ((CastlingSide::Queenside as u8) << BLACK_SIDE_OFFSET));
    }
    ...
    fn test_disable_full_side() {
        ...
        assert_eq!(rights.get(), (CastlingSide::Both as u8) << BLACK_SIDE_OFFSET);
    ...
    fn test_disable_part_of_side() {
        ...
        assert_eq!(rights.get(), (CastlingSide::Queenside as u8) | ((CastlingSide::Both as u8) << BLACK_SIDE_OFFSET));

        rights.disable_part_of_side(Side::Black, CastlingSide::Queenside);
        assert_eq!(rights.get(), (CastlingSide::Queenside as u8) | (CastlingSide::Kingside as u8) << BLACK_SIDE_OFFSET);
    }
    #[test]
    fn test_disable_all() {
        let mut rights = CastlingRights::new();
